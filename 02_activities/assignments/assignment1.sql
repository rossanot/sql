...
SELECT vendor_id, COUNT(vendor_id) AS times FROM vendor_booth_assignments
GROUP BY vendor_id;
...
-- An alternative:
-- SELECT * FROM customer as c
-- INNER JOIN (SELECT customer_id, SUM(quantity*cost_to_customer_per_qty) AS spent FROM customer_purchases
...
SELECT customer_id, strftime('%m', market_date) AS month, strftime('%Y', market_date) AS year, SUM(quantity*cost_to_customer_per_qty) AS spent
FROM customer_purchases
